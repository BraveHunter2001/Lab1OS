...
namespace Lab1OS
{
	class FileManager
	{
		...
		protected static extern IntPtr CreateFile([MarshalAs(UnmanagedType.LPUTF8Str)] string fileName,
									  uint desiredAccess, uint shareMode, winapiFlags.SECURITY_ATTRIBUTES securityAttributes,
		...
		public void CreateFile(bool allowInterrupt)
		{
			...
			var handle = CreateFile(targetPath, (uint)DesiredAccess.GENERIC_WRITE | (uint)DesiredAccess.GENERIC_READ,
						...
						null, (uint)CreationDisposition.CREATE_ALWAYS, (uint)winapiFlags.FileAttributes.FILE_ATTRIBUTE_NORMAL,
						IntPtr.Zero);
		...
		public void PrintFileAttributes(string targetPath)
		{
			...
			if (attributes != INVALID_FILE_ATTRIBUTES)
				foreach (var f in Helper.ParseFlags<winapiFlags.FileAttributes>(attributes))
					Console.WriteLine("\t-" + f);
			else
				Console.WriteLine($"Error occured. Error code {GetLastError()}");
		}

		public void SetFileAttributes(string targetPath, winapiFlags.FileAttributes setAttribute)
		{
